...
type MasterPlanPageProps = { locale: Locale; copy: Dictionary };

export function MasterPlanPage({ locale, copy }: MasterPlanPageProps) {
  return (
    <div className="min-h-screen bg-cream text-ink" lang={locale}>
      <header className="mx-auto flex max-w-2xl items-center justify-between gap-4 px-6 py-6">
        ...
        <div className="flex items-center gap-3">
          <LanguageSwitcher
            locale={locale}
            englishHref={localePath("en", "/master-plan")}
            thaiHref={localePath("th", "/master-plan")}
      ...
      <main className="mx-auto max-w-2xl px-6 pb-28 pt-10 sm:pt-16">
        <p className="hero-rise font-mono text-xs uppercase tracking-[0.28em] text-crimson">{copy.plan.title}</p>
        ...
        <div className="mt-14 space-y-7 text-lg leading-relaxed text-ink/80">
          <p>{copy.plan.paragraphs[0]}</p>
          <p>{copy.plan.paragraphs[1]}</p>
          <p>{copy.plan.paragraphs[2]} <span className="font-semibold text-ink">{copy.plan.emphasis}</span></p>
        </div>

        <hr className="mt-16 border-t-2 border-ink/15" />
        <h2 className="mt-12 font-display text-xl font-extrabold uppercase tracking-tight">{copy.plan.short}</h2>
        <ol className="mt-10 space-y-10">
          {copy.plan.steps.map(([title, body], index) => (
        ...
        <div className="mt-20 border-t-2 border-ink/15 pt-10">
          <p className="font-display text-xl font-extrabold tracking-tight">
            {copy.plan.closing} <span className="text-crimson">{copy.plan.closingHighlight}</span>
          </p>
          <a
          ...
          >
            {copy.plan.join}<span className="transition-transform duration-200 group-hover:translate-x-1">→</span>
          </a>
        </div>
      </main>
    </div>
